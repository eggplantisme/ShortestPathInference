...
def run_1(G, p, shortest_path_times, perturbation_times, save_path, method=None, rtheta_coords=None, network_path=None, gamma=None, T=None, R=None, metric="overlap"):
...
def main2():
    ...
    my_exp.perturbation_times = 5  # 5
    # my_exp.metric = "edit_distance"
    my_exp.metric = "all"
    my_exp.multiprocessing = True
    ...
    if my_exp.method == "hyperbolic_geodesic_distance":
        ...
        with open(real_coords_path, 'r') as f:
            ...
            for line in lines:
                ...
                theta = float(line[2])
                my_exp.rtheta_coords[i] = (r, theta)
    elif my_exp.method.startswith("perturbated_hyperbolic") or my_exp.method == "all":
        # use hyperlink to embed hyperbolic coordinates in perturbated network, given gamma, T and p
        my_exp.gamma = 2.5
        my_exp.T = 0.5
        my_exp.R = 13.604
